...
# ─────────────────────────────────────────────
# STEP 6: Generate Unique VM Name
# ─────────────────────────────────────────────
echo "🎲 Finding available VM name..."
for i in {1..20}; do
  RAND=$(printf "%02d" $((RANDOM % 99 + 1)))
  VM_NAME="modeltraining${RAND}"
  EXISTS=$(gcloud compute instances list --filter="name=$VM_NAME" --format="value(name)" --project="$PROJECT_ID")
  if [ "$EXISTS" != "$VM_NAME" ]; then
...
if [ "$EXISTS" == "$VM_NAME" ]; then
  echo "❌ Could not find a unique VM name after 20 attempts."
  exit 1
fi

# ─────────────────────────────────────────────
# STEP 7: Create VM with Custom Config
# ─────────────────────────────────────────────
...
gcloud compute instances create "$VM_NAME" \
  ...
  --create-disk=auto-delete=yes,boot=yes,device-name="$VM_NAME",image=projects/ubuntu-os-cloud/global/images/ubuntu-minimal-2410-oracular-amd64-v20250606,mode=rw,size=100,type=pd-balanced \
